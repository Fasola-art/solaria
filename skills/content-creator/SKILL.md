...
description: "블로그 본문·브랜드 보이스 분석·SNS 원본 생성·콘텐츠 캘린더 전담. Python 자산(brand_voice_analyzer.py, seo_optimizer.py) 보유. team-marketing Copy & Content Chief 하위. 랜딩·세일즈 카피/카피 편집/이메일 시퀀스/콜드 이메일/SNS 리퍼포징은 marketing-skills 플러그인 위임(SSOT). '블로그', '블로그 본문', '브랜드 보이스 분석', 'SNS 원본', '콘텐츠 캘린더' 키워드로 호출."
license: MIT
metadata:
  version: 1.1.0
...
# Content Creator (축소된 역할)

블로그 본문·브랜드 보이스·SNS 원본 전담. team-marketing 6-Role 체계에서 Copy & Content Chief가 관리.

## 역할 경계 (SSOT, team-marketing/references/policies.md 참조)

| 담당 | 위임 |
|---|---|
| 블로그 본문 작성 | ← 본 스킬 |
| 브랜드 보이스 분석 (brand_voice_analyzer.py) | ← 본 스킬 |
| SEO 글 단위 최적화 (seo_optimizer.py) | ← 본 스킬. 사이트 단위는 marketing-skills:seo-audit |
| 일반 콘텐츠 캘린더 | ← 본 스킬 |
| SNS 원본 생성 | ← 본 스킬 |
...
## Keywords
블로그 본문, 블로그 글, 브랜드 보이스 분석, 브랜드 가이드라인, SEO 글 최적화, SNS 원본, 콘텐츠 캘린더, 콘텐츠 프레임워크, 블로그 템플릿
(제외: 카피 작성/편집, 이메일, 세일즈, SNS 리퍼포징 — marketing-skills 위임)
